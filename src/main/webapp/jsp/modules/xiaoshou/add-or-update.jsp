            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑销售订单</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="kehuId" name="kehuId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>销售数量</label>
                                        <input id="xiaoshouNumber" name="xiaoshouNumber" class="form-control"
                                               onchange="xiaoshouNumberChickValue(this)"  placeholder="销售数量">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#kehuId") !=null){
            ...
            httpJson("xiaoshou/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addxiaoshou');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                goodsId: "required",
                kehuId: "required",
                xiaoshouNumber: "required",
                insertTime: "required",
            },
            messages: {
                goodsId: "商品不能为空",
                kehuId: "客户不能为空",
                xiaoshouNumber: "销售数量不能为空",
                insertTime: "销售时间不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addxiaoshou = window.sessionStorage.getItem("addxiaoshou");
        if (addxiaoshou != null && addxiaoshou != "" && addxiaoshou != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("xiaoshou/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#xiaoshouNumber").val(ruleForm.xiaoshouNumber);
